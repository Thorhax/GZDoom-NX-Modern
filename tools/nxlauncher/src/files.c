...
{
    static char buf[512];
    snprintf(buf, sizeof(buf), "%s/%s", fs_basedir, fs_profiles[g].iwad);
    if (fexists(buf)) return 1;
    snprintf(buf, sizeof(buf), "%s/iwads/%s", fs_basedir, fs_profiles[g].iwad);
    if (fexists(buf)) return 1;
    snprintf(buf, sizeof(buf), "%s", fs_profiles[g].iwad);
    if (fexists(buf)) return 1;
    snprintf(buf, sizeof(buf), "iwads/%s", fs_profiles[g].iwad);
    return fexists(buf);
}
...
int FS_Init(void)
{
    ...
    if (fexists("gzdoom.nro") || fexists("launcher.ini") || fexists("iwads") || fexists("doom.wad") || fexists("doom2.wad") || fexists("doom1.wad") || fexists("freedoom1.wad") || fexists("freedoom2.wad"))
    {
        strcpy(fs_basedir, ".");
    }
    else
    {
        strcpy(fs_basedir, BASEDIR);
    }

    mkdir(fs_basedir, 0755);
    char tmp[128];
    snprintf(tmp, sizeof(tmp), "%s/" RELATIVE_TMPDIR, fs_basedir);
    mkdir(tmp, 0755);
    snprintf(tmp, sizeof(tmp), "%s/pwads", fs_basedir);
    mkdir(tmp, 0755);
    snprintf(tmp, sizeof(tmp), "%s/iwads", fs_basedir);
    mkdir(tmp, 0755);
    snprintf(tmp, sizeof(tmp), "%s/save", fs_basedir);
    mkdir(tmp, 0755);
...
{
    char buf[512];
    snprintf(buf, sizeof(buf), "%s/launcher.ini", fs_basedir);
    FILE *f = fopen(buf, "r");
    if (!f) return -666;
...
void FS_ExecGame(int game)
{
    ...
    if (fexists("gzdoom.nro"))
        snprintf(exe, sizeof(exe), "%s/gzdoom.nro", fs_cwd);
    else if (fexists(BASEDIR "/gzdoom.nro"))
        snprintf(exe, sizeof(exe), "%s/" BASEDIR "/gzdoom.nro", fs_cwd);
    else
        snprintf(exe, sizeof(exe), "%s/%s/gzdoom.nro", fs_cwd, fs_basedir);
