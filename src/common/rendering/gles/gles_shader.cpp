...
bool FShader::Load(const char * name, const char * vert_prog_lump_, const char * frag_prog_lump_, const char * proc_prog_lump_, const char * light_fragprog_, const char * defines)
{
	...
	if (proc_prog_lump.Len() && fileSystem.CheckNumForFullName(proc_prog_lump.GetChars(), 0) == -1)
	{
		FString alt = proc_prog_lump;
		alt.Substitute("shaders/", "shaders_gles/");
		if (fileSystem.CheckNumForFullName(alt.GetChars(), 0) != -1)
		{
			proc_prog_lump = alt;
		}
	}
...
//
	...
	int texindex = glGetUniformLocation(shaderData->hShader, "tex");
	if (texindex >= 0) glUniform1i(texindex, 0);
...
FShader *FShaderCollection::Get(unsigned int eff, bool alphateston)
{
	// indices 0-2 match the warping modes, 3 no texture, the following are custom
	if (!alphateston && eff <= 2 && eff < mMaterialShadersNAT.Size() && mMaterialShadersNAT[eff] != nullptr)
	{
		return mMaterialShadersNAT[eff];
	}
	if (eff >= FIRST_USER_SHADER)
	{
		int userIndex = eff - FIRST_USER_SHADER;
		if (userIndex < (int)usershaders.Size())
		{
			if (eff >= mMaterialShaders.Size())
			{
				unsigned int oldSize = mMaterialShaders.Size();
				mMaterialShaders.Resize(eff + 1);
				for (unsigned int i = oldSize; i <= eff; i++) mMaterialShaders[i] = nullptr;
			}
			if (mMaterialShaders[eff] == nullptr)
			{
				FString name = ExtractFileBase(usershaders[userIndex].shader.GetChars());
				FString defines = defaultshaders[usershaders[userIndex].shaderType].Defines + usershaders[userIndex].defines;
				mMaterialShaders[eff] = Compile(name.GetChars(), usershaders[userIndex].shader.GetChars(), defaultshaders[usershaders[userIndex].shaderType].lightfunc, defines.GetChars(), true, mPassType);
			}
			if (mMaterialShaders[eff] != nullptr)
			{
	...
	if (eff < mMaterialShaders.Size() && mMaterialShaders[eff] != nullptr)
	{
		return mMaterialShaders[eff];
	}
	else
